...
const authSyncControllerFactory = window.TaskNotesAuthSyncController;
const tagUtils = window.TaskNotesTagUtils;
const syncMerge = window.TaskNotesSyncMerge;

if (!authSyncControllerFactory) {
  throw new Error('TaskNotesAuthSyncController module is required before renderer.js');
}
...
const {
  ...
  clearCloudRetry,
  scheduleCloudRetry
} = authSyncControllerFactory.create({
  state,
  refs,
